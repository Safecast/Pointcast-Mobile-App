...
cocos2d::ui::Widget *
Chart::prepareHeader(scene::menu::Analytics *p_scene_analytics,
  ...
  // google map icon
  auto google_map_icon_image = p_background->getChildByName<ui::ImageView *>("imgGoogleMapIcon");
  google_map_icon_image->addTouchEventListener(
  [location_item](Ref *sender, ui::Widget::TouchEventType type) {
    if (type == ui::Widget::TouchEventType::ENDED) {
      log("google_map_icon_image touched");
      // ロケーションをシングルトンにセット
      lib::network::DataStoreSingleton *p_data_store_singleton =
        lib::network::DataStoreSingleton::getInstance();
      p_data_store_singleton->setLocation(location_item.latitude, location_item.longitude);
      // マップをタップしたことにする
      cocos2d::EventCustom customEvent("touch_map");
...
void Chart::prepareChart(
    cocos2d::ui::Widget *p_chart_board_widget,
    scene::menu::Analytics *p_scene_analytics, int m_sensor_main_id,
    const std::vector<lib::object::ChartItem> v_chart_items,
    const std::vector<lib::object::WeatherItem> v_weather_items,
    bool is_empty) {

  // get chart config
  scene::chart::Board::Config chart_config =
      Chart::getConfig(v_chart_items, v_weather_items, p_scene_analytics,
                       m_sensor_main_id, is_empty);
  ...
  cocos2d::log("p_chart_board_widget size %f, %f pos %f, %f",
               p_chart_board_widget->getContentSize().width,
               p_chart_board_widget->getContentSize().height,
               p_chart_board_widget->getPositionX(),
               p_chart_board_widget->getPositionY());
}

scene::chart::Board::Config
Chart::getConfig(const std::vector<lib::object::ChartItem> &v_chart_items,
                 const std::vector<lib::object::WeatherItem> &v_weather_items,
                 scene::menu::Analytics *p_scene_analytics,
                 int m_sensor_main_id, bool is_empty) {
...
void Chart::drawDottedLine(cocos2d::DrawNode *p_node, float line_width,
                           float line_interval, Point start_point,
                           Point end_point, float interval,
