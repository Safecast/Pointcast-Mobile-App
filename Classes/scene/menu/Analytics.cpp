...
bool Analytics::init() {
  ...
  Director::getInstance()->getEventDispatcher()->addCustomEventListener(
      "orientation", [=](cocos2d::EventCustom *event) {
        CCLOG("イベント受け取ったよ > %s", event->getEventName().c_str());
        this->onDidOrientation();
      });

  this->_p_store_callback =
      CallFunc::create(CC_CALLBACK_0(Analytics::onCallbackDataStore, this));

  /*
  this->_pinch_listener = EventListenerTouchAllAtOnce::create();
  this->_pinch_listener->setEnabled(true);
  this->_pinch_listener->onTouchesBegan =
  CC_CALLBACK_2(Analytics::onTouchesBegan, this);
  this->_pinch_listener->onTouchesMoved =
  CC_CALLBACK_2(Analytics::onTouchesMoved, this);
  this->_pinch_listener->onTouchesCancelled =
  CC_CALLBACK_2(Analytics::onTouchesCancelled, this);
  this->_pinch_listener->onTouchesEnded =
  CC_CALLBACK_2(Analytics::onTouchesEnded, this);
  this->getEventDispatcher()->addEventListenerWithSceneGraphPriority(this->_pinch_listener,
  this);
  */
  return true;
}
...
void Analytics::initFixedContents() {
  ...
  if (this->isPortlate()) {
    this->_p_contents = cocos2d::CSLoader::getInstance()->createNode(
        "res/MenuAnalytics_portlate.csb");
  } else {
    this->_p_contents = cocos2d::CSLoader::getInstance()->createNode(
        "res/MenuAnalytics_landscape.csb");
  ...
  auto label_back =
      this->_p_panel_background->getChildByName<ui::Text *>("txtBack");
  ...
  // favorite
  this->_p_btn_favorite =
      this->_p_panel_background->getChildByName<ui::Button *>("btnFavorite");
  ...
  cocos2d::log("size w %f h %f x %f y %f", this->getContentSize().width,
               this->getContentSize().height, this->getPositionX(),
               this->getPositionY());
  ...
  this->_p_next_button =
      this->_p_panel_background->getChildByName<ui::Button *>("NextButton");
  this->_p_next_button->addTouchEventListener(
      [this](Ref *sender, ui::Widget::TouchEventType type) {
        ...
        if (type == ui::Widget::TouchEventType::ENDED) {
          ...
          this->updateChartWithDiffDays(diff_days);
        }
      });
  this->_p_prev_button =
      this->_p_panel_background->getChildByName<ui::Button *>("PrevButton");
  ...
  this->_prev_button->setParam(prev_button_size, Color3B(111,201,88), 10, 10,
  prev_str, Color3B::WHITE, 24);
  ...
  this->_next_button->setParam(next_button_size, Color3B(111,201,88), 10, 10,
  next_str, Color3B::WHITE, 24);
  ...
  listener->onTouchBegan = [this](Touch* touch, Event* event){
      ...
      cocos2d::log("_prev_button rect : (X1,X2,Y1,Y2)(%f,%f,%f,%f)",
                   this->_prev_button->getPositionX(),this->_prev_button->getPositionX()
  + this->_prev_button->getContentSize().width,
  this->_prev_button->getPositionY(),this->_prev_button->getPositionY() +
  this->_prev_button->getContentSize().height);
      cocos2d::log("_next_button rect : (X1,X2,Y1,Y2)(%f,%f,%f,%f)",
                   this->_next_button->getPositionX(),this->_next_button->getPositionX()
  + this->_next_button->getContentSize().width,
  this->_next_button->getPositionY(),this->_next_button->getPositionY() +
  this->_next_button->getContentSize().height);
...
void Analytics::initVariableContents() {
  ...
  // draw header
  auto p_header = scene::layout::helper::Chart::prepareHeader(
      this, this->_m_sensor_main_id);
...
void Analytics::drawChart() {
  ...
  const std::string analytics_data =
      p_data_store_singleton->getResponseAnalyticsData(
          this->_m_sensor_main_id, this->_interval_start, this->_interval_end);

  this->_current_cache_key = p_data_store_singleton->getAnalyticsCacheKey(
      this->_m_sensor_main_id, this->_interval_start, this->_interval_end);
  ...
  scene::layout::helper::Chart::prepareChart(this->_p_chart_widget, this,
                                             this->_m_sensor_main_id,
                                             v_chart_items, v_weather_items);
  ...
  bool button_state =
      (now > this->_interval_start && now < this->_interval_end) ? false : true;
...
void Analytics::onEnter() {
  ...
  this->_p_scene_main->attachWaitAnimation();
  p_data_store_singleton->storeAnalyticsData(
      this->_m_sensor_main_id, this->_interval_start, this->_interval_end, true,
      this->_p_store_callback);
...
void Analytics::close() {
  ...
  if (!this->isPortlate()) {
    ...
    lib::native::Util::changeRotate(
        lib::native::Util::UIDeviceOrientationPortrait);
...
int Analytics::getMSensorMainId(void) { return this->_m_sensor_main_id; }
...
bool Analytics::isPortlate() { return this->_portlate; }
...
time_t Analytics::getIntervalStart() { return this->_interval_start; }

time_t Analytics::getIntervalEnd() { return this->_interval_end; }

void Analytics::onTouchesBegan(const std::vector<cocos2d::Touch *> &touches,
...
void Analytics::onTouchesMoved(const std::vector<cocos2d::Touch *> &touches,
                               cocos2d::Event *pEvent) {
...
void Analytics::onTouchesEnded(const std::vector<cocos2d::Touch *> &touches,
                               cocos2d::Event *pEvent) {
  ...
  for(int i=0; i < touches.size(); i++){
      ...
      //touchPoint : タッチされた場所
      Point touch_point = Vec2(touches.at(i)->getLocationInView().x,
  touches.at(i)->getLocationInView().y);
...
void Analytics::onTouchesCancelled(const std::vector<cocos2d::Touch *> &touches,
                                   cocos2d::Event *pEvent) {}

bool Analytics::isGesture(const std::vector<cocos2d::Touch *> &touches,
                          cocos2d::Event *pEvent) {
  //
...
void Analytics::updateChartScale() {
  this->_p_chart_nodes[this->_current_cache_key]->setScale(
      this->_current_scale);
  this->_p_chart_scroll_view->setInnerContainerSize(
      this->_p_chart_nodes[this->_current_cache_key]->getBoundingBox().size);
  this->_p_chart_nodes[this->_current_cache_key]->setPositionY(0.0f);
}
...
/*
void Analytics::pageViewEvent(cocos2d::Ref *pSender,
...
void Analytics::updateChartWithDiffDays(int diff_days) {

  // 日付を1日戻す
  this->shiftIntervalSec(diff_days * 86400);
...
void Analytics::retryRequest() {
  // リクエスト失敗時にリトライする
  cocos2d::log("Analytics::retryRequest()");
  int diff_days = 0;
  this->updateChartWithDiffDays(diff_days);
}

void Analytics::retryCancel() {
  // リトライしない場合アプリケーション終了
